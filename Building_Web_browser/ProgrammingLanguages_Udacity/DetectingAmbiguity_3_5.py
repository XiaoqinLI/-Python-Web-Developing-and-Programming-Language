...
#----------recursion dfs and get each path--------------
def isambig(grammar, start, utterance):
    ...
    def dfs(current, path):
        ...
        for pos, item in enumerate(current):
            for idx, (key, value) in enumerate(grammar):
                if key == item:
                    rewrite = current[:pos] + value + current[pos+1:]
                    dfs(rewrite, path + [(pos, idx)]) # pos: ith in "current" list, idx: ith tuple in grammar
    dfs([start], [])
    for path in paths:
        print path
    return len(paths) > 1
...
#----------recursion expanding dfs2-----------------
# def isambig(grammar, start, utterance):
#
#     all_utterances = []
#
#     def expand(current):
#         all_utterances.append(current)
#         for pos, token in enumerate(current):
#             for rule in grammar:
#                 if token == rule[0]:
#                     rewrite = current[:pos] + rule[1] + current[pos+1:]
#                     expand(rewrite)
#
#     expand([start])
#     for utter in all_utterances:
#         print utter
#     return all_utterances.count(utterance) > 1

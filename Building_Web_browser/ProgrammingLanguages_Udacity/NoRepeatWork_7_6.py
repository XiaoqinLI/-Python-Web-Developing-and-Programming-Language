...
def optimize(ast):
    ...
    for i in range(len(ast)):
        type, var, exp = ast[i]
        if exp[0] == "binop":
            nono_list = [exp[1][1], exp[3][1]]

            for j in range(i-1, -1, -1):
                if ast[j][1] in nono_list:
                    break
                elif ast[j][2] == exp:
                    ast[i] = (type, var, ("identifier",lookup[exp][0]))
            if exp in lookup:
                lookup[exp] += [var]
            else:
                lookup[exp] = [var]

        elif exp[0] == "number" or exp[0] == "identifier":
            for key,val in lookup.items():
                if var in val:
                    lookup[key].remove(var)
...
print optimize(example0) == answer0

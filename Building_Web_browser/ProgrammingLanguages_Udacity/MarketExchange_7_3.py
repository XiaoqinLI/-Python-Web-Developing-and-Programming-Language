...
def evaluate(ast):
    ...
    while(flag):
        ...
        for entry in ast:
            if entry[1] == "buy":
                for entry1 in ast:
                    if entry1[1] == "sell" and  entry1[2] == entry[2] and entry1[3] == entry[3] and info[entry[0]] >= entry[3]:
                        info[entry[0]] -= entry[3]
                        info[entry1[0]] += entry1[3]
                        ast.remove(entry)
                        ast.remove(entry1)
        if len(ast) == lenth:
            flag = False
...
test4 = [ ["klaus","has",50] ,
          ...
          ["wrede","buy","wheat", 25] ,
          ["wrede","buy","wheat", 25] ,
          ["andreas","sell","wheat", 25] ,
          ["klaus","sell","wheat", 25] ,
          ]
print evaluate(test4) == {'andreas': 75, 'klaus': 75, 'wrede': 0}